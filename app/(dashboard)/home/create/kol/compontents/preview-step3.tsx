...
export default function PreviewStepThree() {
  ...
  const [isRevealingComments, setIsRevealingComments] =
  ...
  const parseContent = (
    ...
    if (commentsMatch && commentsMatch[1]) {
      // 分割评论
      const commentLines = commentsMatch[1]
        .split(/\d+\./)
  ...
  return (
    <div className="px-4 space-y-4 text-md">
      ...
      {displayedComments.length > 0 && (
        <>
          <PreviewLoader
            text={`Generated Comments (${displayedComments.length}${
              message ? "/" + message.comments.length : ""
            }):`}
