...
export default function HomePage() {
  ...
  const userInfoDetails = useAppSelector(
    (state) => state.userReducer.userInfo.details
  );

  const handleCreateKol = () => {
    if (
      isLoggedIn &&
      userInfoDetails.agent.created < userInfoDetails.agent.total
    ) {
          ...
          <div className="flex justify-center items-center w-full h-full flex-col space-y-6">
            ...
            <dl className="text-center space-y-2">
              ...
              <dd className="text-base text-muted-foreground">
                {agents.length > 0
                  ? "Simply click the button above to create your agent."
            ...
            >
              {agents.length > 0 ? "Create agent" : "Create your first agent"}
            </Button>
          </div>
        </div>
      </div>
